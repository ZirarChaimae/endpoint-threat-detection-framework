...
RESPONSE_LOG_FILE = REPO_ROOT / "correlation" / "incidents" / "response-log.json"


def incident_key(incident):
    """A stable identifier for an incident that survives across hunt runs,
    since correlate.py regenerates a fresh incidents_*.json file every time
    but the underlying event timestamps (derived from real log data) don't
    change. This lets a response action recorded against an incident in one
    hunt still be recognized as 'the same incident' in a later hunt's file."""
    return f"{incident.get('pattern_id')}|{incident.get('start_time')}|{incident.get('computer')}"


def load_response_log():
    if not RESPONSE_LOG_FILE.exists():
        return []
    try:
        with open(RESPONSE_LOG_FILE, encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return []


def save_response_log(entries):
    RESPONSE_LOG_FILE.parent.mkdir(exist_ok=True)
    with open(RESPONSE_LOG_FILE, "w", encoding="utf-8") as f:
        json.dump(entries, f, indent=2)


def append_response_log_entry(incident, entry):
    log = load_response_log()
    entry = dict(entry)
    entry["incident_key"] = incident_key(incident)
    entry["executed_at"] = datetime.now().isoformat()
    log.append(entry)
    save_response_log(log)


def enrich_incidents_with_response_history(incident_list):
    log = load_response_log()
    for incident in incident_list:
        key = incident_key(incident)
        matches = [e for e in log if e.get("incident_key") == key]
        matches.sort(key=lambda e: e.get("executed_at", ""), reverse=True)
        incident["response_history"] = matches
...
def incidents():
    try:
        ...
        with open(files[0], encoding="utf-8") as f:
            data = json.load(f)
        data = enrich_incidents_with_response_history(data)
...
def generate_incident_report_markdown(incident):
    ...
    history = incident.get("response_history") or []
    if history:
        lines.append("### Execution History")
        lines.append("")
        lines.append("| Executed At | Mode | Action | Target | Result | Output |")
        lines.append("|---|---|---|---|---|---|")
        for h in history:
            mode = "DRY RUN" if h.get("dry_run") else "LIVE"
            result = "Success" if h.get("success") else "Failed"
            output_short = (h.get("output") or "")[:80]
            lines.append(f"| {h.get('executed_at','')} | {mode} | {h.get('action','')} | {h.get('target','')} | {result} | {output_short} |")
        lines.append("")
...
def respond():
    ...
    try:
        ...
        if rule["type"] == "kill_process":
            ...
            result = {"success": success, "action": "kill_process", "target": pid,
                      "dry_run": dry_run, "verified": verified, "output": output}
            append_response_log_entry(incident, result)
            ...
            result = {"success": success, "action": "block_ip", "target": ip,
                      "dry_run": dry_run, "verified": verified, "output": output}
            append_response_log_entry(incident, result)
            return jsonify(result)
